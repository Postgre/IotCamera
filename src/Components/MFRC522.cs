...
namespace Components
{
    public class MFRC522
    {
        ...
        public MFRC522()
        {
            Spi.Channel0Frequency = 500000;
            spi = Spi.Channel0;

            var nrstdpPin = Gpio[NRSTPD];
        ...
        {
            var val = spi.SendReceive(new byte[] {
        ...
        private (byte status, byte[] backData, byte backLen) ToCard(byte command, byte[] sendData)
        {
            ...
            while (true)
            {
                ...
                i -= 1;
                if (Convert.ToBoolean(~Convert.ToInt32((
                    (i != 0) && Convert.ToBoolean(~(n & 0x01)) && Convert.ToBoolean(~(n & waitIRq))
                    ))))
                {
                    break;
                }
            ...
            if (i != 0)
            {
                if ((ReadSpi(ErrorReg) & 0x1B) == 0x00)
                {
                    ...
                    if (command == PCD_TRANSCEIVE)
                    {
                        ...
                        lastBits = (byte)(ReadSpi(ControlReg) & 0x07);
                        if (lastBits != 0)
                        {
        ...
        public (byte status, int backBits) Request(byte reqMode)
        {
        ...
        public (byte status, byte[] backData) Anticoll()
        {
            byte serNumCheck = 0;
            ...
            var (status, backData, backBits) = ToCard(PCD_TRANSCEIVE, serNum.ToArray());
        ...
        private byte[] CalulateCRC(byte[] pIndata)
        {
            ...
            pOutData.Add(ReadSpi(CRCResultRegL));
            pOutData.Add(ReadSpi(CRCResultRegM));
        ...
        public byte SelectTag(byte[] serNum)
        {
            ...
            buf.Add(pOut[0]);
            buf.Add(pOut[1]);
            var (status, backData, backLen) = ToCard(PCD_TRANSCEIVE, buf.ToArray());

            if (status == MI_OK && backLen == 0x18)
            {
                Console.WriteLine($"Size: {backData[0]}");
                return backData[0];
            }
            else
            {
        ...
        public byte Auth(byte authMode, byte BlockAddr, byte[] Sectorkey, byte[] serNum)
        {
            ...
            var (status, backData, backLen) = ToCard(PCD_AUTHENT, buff.ToArray());
            ...
            {
                Console.WriteLine("AUTH ERROR(status2reg & 0x08) != 0");
            }
        ...
        private void Write(byte blockAddr, byte[] writeData)
        {
            ...
            Console.WriteLine($"{backLen} backdata &0x0F == 0x0A {backData[0] & 0x0F}");
            if (status == MI_OK)
            {
                int i = 0;
                var buf = new List<byte>();
